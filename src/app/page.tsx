...
export default function Home() {
	...
	// 1) state
	const [awaitingBridgeAfterApprove, setAwaitingBridgeAfterApprove] = useState(false)
	const [bridgeTxHash, setBridgeTxHash] = useState<`0x${string}` | undefined>(undefined)
	// 新增：按钮点击后立即进入 loading 的本地 UI 覆盖
	const [uiLoadingAction, setUiLoadingAction] = useState<null | 'approve' | 'bridge'>(null)
	// 新增：记录已处理的授权交易哈希，避免重复触发
	const [lastHandledApproveHash, setLastHandledApproveHash] = useState<`0x${string}` | null>(null)
	...
	const getButtonState = () => {
		...
		if (!termsAccepted) {
			return { text: 'Accept Terms', disabled: true, action: 'none' }
		}

		// 优先使用本地 UI 覆盖，点击即进入 loading
		if (uiLoadingAction === 'bridge') {
			return { text: 'Bridging...', disabled: true, action: 'bridge' }
		}
		if (uiLoadingAction === 'approve') {
			return { text: 'Approving...', disabled: true, action: 'approve' }
		}
		...
		// 链上状态指示（兜底）
		if ((isBridgePending || isBridgeConfirming) && bridgeTxHash) {
			return { text: 'Bridging...', disabled: true, action: 'bridge' }
		}

		// 2) 未选 token / 未输入金额 / 余额不足
		if (!fromToken) return { text: 'Select Token', disabled: true, action: 'none' }
		if (!fromAmount || parseFloat(fromAmount) <= 0) return { text: 'Enter Amount', disabled: true, action: 'none' }
		if (Number(balanceWei) < amountWei) return { text: 'Insufficient Balance', disabled: true, action: 'none' }

		// 授权不足 → Approving/Approve
	...
	// 	setAwaitingBridgeAfterApprove(false) // 重置标志
	...
	const handleApprove = useCallback(() => {
		if (fromToken?.address && fromNetwork?.contract) {
			try {
				setAwaitingBridgeAfterApprove(true) // 设置标志
				setUiLoadingAction('approve')
				writeApprove({
					address: fromToken.address as `0x${string}`,
					abi: ERC20_ABI,
	...
	}, [fromToken, fromNetwork, writeApprove, setAwaitingBridgeAfterApprove]);

	// 只在“来自按钮的Approve完成”后自动 Bridge
	useEffect(() => {
		// 仅当：授权已确认 + 有有效哈希 + 该哈希未被处理过 + 是由按钮发起的授权
		if (isApproveConfirmed && approveHash && approveHash !== lastHandledApproveHash && awaitingBridgeAfterApprove) {
			toast.success('Approve confirmed')
			// 授权成功后刷新 allowance，确保按钮文案及时切换
			refetchTokenAllowance?.()
			handleBridge()
			setAwaitingBridgeAfterApprove(false) // 执行后清标志
			setLastHandledApproveHash(approveHash)
		}
	}, [isApproveConfirmed, approveHash, lastHandledApproveHash, awaitingBridgeAfterApprove, handleBridge, refetchTokenAllowance])

	// 当网络/代币变更时，重置已处理授权哈希，避免旧哈希在新环境下误触发
	useEffect(() => {
		setLastHandledApproveHash(null)
		setAwaitingBridgeAfterApprove(false)
	...
	}, [isBridgeConfirmed, bridgeTxHash, refetchTokenBalance])
	...
	}, [isBridgeError, bridgeTxHash])

	// 监听用户取消（关闭弹窗/拒签）：pending -> false 且无 success/error
	useEffect(() => {
		if (uiLoadingAction === 'approve' && !isApprovePending && !isApproveConfirmed && !isApproveError) {
			setUiLoadingAction(null)
			setAwaitingBridgeAfterApprove(false)
		}
	}, [uiLoadingAction, isApprovePending, isApproveConfirmed, isApproveError])

	useEffect(() => {
		if (uiLoadingAction === 'bridge' && !isBridgePending && !isBridgeConfirmed && !isBridgeError && !bridgeTxHash) {
			setUiLoadingAction(null)
		}
	}, [uiLoadingAction, isBridgePending, isBridgeConfirmed, isBridgeError, bridgeTxHash])
	...
	}, [buttonState.action, handleApprove, handleBridge]);
	...
	}, [currentChainId, switchChain, setAwaitingBridgeAfterApprove])
